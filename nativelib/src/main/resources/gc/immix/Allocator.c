...
BlockHeader *Allocator_getNextBlock();
bool Allocator_getNextLine();
bool Allocator_newBlock();
 ...
 * the block to alloc.
 */
word_t *Allocator_overflowAllocation(size_t size) {
    ...
    if (end > allocator.largeLimit) {
        ...
        return Allocator_overflowAllocation(size);
    }
 ...
 */
INLINE word_t *Allocator_Alloc(size_t size) {
 ...
 * the recycled block
 */
bool Allocator_nextLineRecycled() {
...
bool Allocator_getNextLine() {
    ...
    if (allocator.cursor == NULL || BlockHeader_IsFree(allocator.block)) {
        ...
        return Allocator_nextLineRecycled();
    }
}
